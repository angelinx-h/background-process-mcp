...
const App = ({ serverUrl, isDebugMode }) => {
    const { stdout } = useStdout();
    const { exit } = useApp();
    const [isWide, setIsWide] = useState(stdout.columns >= RESPONSIVE_BREAKPOINT);
    ...
    const handleMessage = useCallback((data) => {
        try {
            ...
            if (isDebugMode) {
                setDebugLog((prev) => [
                    `[${new Date().toLocaleTimeString()}] RECV: ${JSON.stringify(message)}`,
                    ...prev,
                ].slice(0, 50));
            }
            switch (message.action) {
                case 'server.full_state':
                    setProcesses(message.processes.map((p) => ({ ...p, output: undefined })));
                ...
                case 'server.process_cleared':
                    setProcesses((prevProcesses) => {
                        const newProcesses = prevProcesses.filter((p) => p.id !== message.processId);
                        setSelectedProcessId((prevSelectedId) => {
                            if (prevSelectedId === message.processId) {
                                if (newProcesses.length === 0)
                                    return null;
                                const oldIndex = prevProcesses.findIndex((p) => p.id === message.processId);
                                return newProcesses[Math.min(oldIndex, newProcesses.length - 1)].id;
                            }
                ...
                case 'server.list_processes_response':
                    setProcesses((prevProcesses) => {
                        const clientProcs = new Map(prevProcesses.map((p) => [p.id, p]));
                        return message.processes.map((sp) => ({
                            ...sp,
    ...
    const handleNewProcess = (command) => {
        sendMessage({
    ...
    if (viewMode === 'live_output' && selectedProcess) {
        return (_jsx(LiveOutputView, { process: selectedProcess, terminalHeight: terminalHeight }));
    }
    return (_jsxs(Box, { flexDirection: "column", padding: 1, width: "100%", height: terminalHeight, children: [_jsx(Box, { flexGrow: 1, flexDirection: isWide ? 'row' : 'column', children: isWide ? (_jsxs(_Fragment, { children: [_jsxs(Box, { flexDirection: "column", width: "50%", paddingRight: 1, children: [_jsx(StatusPanel, { isConnected: isConnected, serverVersion: serverVersion, processCount: processes.length, port: serverPort, pid: serverPid, uptime: serverUptime, processCounts: processCounts }), _jsx(Box, { marginTop: 1, flexGrow: 1, children: _jsx(ProcessList, { processes: processes, selectedProcessId: selectedProcessId }) })] }), _jsxs(Box, { flexDirection: "column", width: "50%", children: [_jsx(ProcessDetail, { process: selectedProcess }), _jsx(Box, { marginTop: 1, flexGrow: 1, children: _jsx(ProcessOutput, { process: selectedProcess }) })] })] })) : (_jsxs(_Fragment, { children: [_jsx(StatusPanel, { isConnected: isConnected, serverVersion: serverVersion, processCount: processes.length, port: serverPort, pid: serverPid, uptime: serverUptime, processCounts: processCounts }), _jsx(Box, { marginTop: 1, children: _jsx(ProcessDetail, { process: selectedProcess }) }), _jsx(Box, { marginTop: 1, children: _jsx(ProcessList, { processes: processes, selectedProcessId: selectedProcessId }) }), _jsx(Box, { marginTop: 1, flexGrow: 1, children: _jsx(ProcessOutput, { process: selectedProcess }) })] })) }), isDebugMode && isDebugVisible && _jsx(DebugLog, { log: debugLog }), isInputting ? (_jsx(InputBar, { onSubmit: handleNewProcess, onCancel: () => setIsInputting(false) })) : (_jsx(HelpBar, {}))] }));
};
export default App;
