...
class CoreServiceClient {
    ...
    connectionPromise;
    ownedServer = null;
    constructor(serverUrl, ownedServer = null) {
...
async function main() {
    ...
    mcpServer.addTool({
        ...
        execute: async ({ processId, head, tail }) => {
            ...
            if (result.action === 'server.output_response'
    ...
    mcpServer.addTool({
        ...
        execute: async ({ command }) => {
            ...
            if (startResult.action !== 'server.request_response'
                || !startResult.success
                || !startResult.data) {
            ...
            return new Promise((resolve) => {
                ...
                const messageHandler = (data) => {
                    try {
                        const message = ServerMessageSchema.parse(JSON.parse(data.toString()));
                        if (message.action === 'server.process_output'
                            && message.processId === processId) {
                            output += message.cleanOutput;
                        }
                        if (message.action === 'server.process_stopped'
